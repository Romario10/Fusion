...
int chrif_save(struct map_session_data *sd, int flag) {
	...
	if (flag && sd->state.active) { //Store player data which is quitting
		...
		if ( !chrif_auth_logout(sd,flag == 1 ? ST_LOGOUT : ST_MAPCHANGE) )
			ShowError("chrif_save: Falha em configurar jogador %d:%d para sair corretamente!\n", sd->status.account_id, sd->status.char_id);
	}
...
int chrif_sendmap(int fd) {
	...
	ShowStatus("Mandando mapas para o char-server...\n");
...
int chrif_removemap(int fd) {
	...
	if(battle_config.etc_log)
		ShowStatus("Removido mapas de %d.%d.%d.%d:%d (%d mapas)\n", CONVIP(ip), port, j);
...
int chrif_changemapserverack(int account_id, int login_id1, int login_id2, int char_id, short map_index, short x, short y, uint32 ip, uint16 port) {
	...
	if ( !login_id1 ) {
		ShowError("Mudança de map-server falhou.\n");
...
int chrif_connectack(int fd) {
	...
	ShowStatus("Conectado ao char-server (Conexão: '"CL_WHITE"%d"CL_RESET"').\n",fd);
	chrif_state = 1;
	chrif_connected = 1;
...
int chrif_sendmapack(int fd) {
	...
	if (RFIFOB(fd,2)) {
		ShowFatalError("chrif_sendmapack: falha em enviar lista de mapas ao map-server %d\n", RFIFOB(fd,2));
		exit(EXIT_FAILURE);
	}
...
void chrif_authok(int fd) {
	...
	if( RFIFOW(fd,2) - 25 != sizeof(struct mmo_charstatus) ) {
		ShowError("chrif_authok: Tamanho das informação incompatíveis! %d != %d\n", RFIFOW(fd,2) - 25, sizeof(struct mmo_charstatus));
		return;
	}
...
static void chrif_char_ask_name_answer(int acc, const char* player_name, uint16 type, uint16 answer) {
	...
	if( acc < 0 || sd == NULL ) {
		ShowError("chrif_char_ask_name_answer: falhou, jogador offline.\n");
		return;
	}
...
int chrif_accountban(int fd) {
	...
	if ( acc < 0 || sd == NULL ) {
		ShowError("chrif_accountban: falhou, jogador offline.\n");
		return 0;
	}
...
int chrif_load_scdata(int fd) {
	...
	if ( sd->status.char_id != cid ) {
		ShowError("chrif_load_scdata: Recebendo dados da conta %d, char id incompatível (%d != %d)!\n", aid, sd->status.char_id, cid);
		return -1;
	}
...
int chrif_parse(int fd) {
	...
	while ( RFIFOREST(fd) >= 2 ) {
		...
		if (cmd < 0x2af8 || cmd >= 0x2af8 + ARRAYLENGTH(packet_len_table) || packet_len_table[cmd-0x2af8] == 0) {
			...
			ShowWarning("chrif_parse: sessão #%d, intif_parse falhou (comando 0x%.4x irreconhecível).\n", fd, cmd);
			set_eof(fd);
			return 0;
		}
