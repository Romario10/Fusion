...
int chat_createnpcchat(struct npc_data* nd, const char* title, int limit, bool pub, int trigger, const char* ev, int zeny, int minLvl, int maxLvl)
{
	...
	if( nd->chat_id ) {
		ShowError("chat_createnpcchat: npc '%s' já tem chatroom, não pode criar novo!\n", nd->exname);
		return 0;
	}

	if( zeny > MAX_ZENY || maxLvl > MAX_LEVEL ) {
		ShowError("chat_createnpcchat: npc '%s' tem um nível ou zeny requerido acima do limite!\n", nd->exname);
		return 0;
	}
